...
public class Consultas {
  ...
  public Set<String> atoresQueInterpretaramSiProprios() {
    return this.filmes.stream()
        .flatMap(e -> e.atores.stream().filter(t -> e.personagens.contains(t)))
  ...
  public List<String> atoresQueAtuaramEmFilmesDoDiretorEmOrdemAlfabetica(String diretor) {
    return this.filmes.stream().filter(e -> e.diretores.contains(diretor))
        .flatMap(r -> r.atores.stream()).distinct().sorted().collect(Collectors.toList());
  }
  ...
  public List<Filme> filmesEmQuePeloMenosUmDiretorAtuouMaisRecentesPrimeiro() {
    List<Filme> diretorAtor =
        this.filmes.stream().sorted(Comparator.comparingInt(filme -> -filme.anoDeLancamento))
            .flatMap(e -> e.diretores.stream().filter(t -> e.atores.contains(t)).map(p -> e))
  ...
  public Map<String, Set<Filme>> filmesLancadosNoAnoAgrupadosPorCategoria(int ano) {
    return this.filmes.stream().filter(e -> e.anoDeLancamento == ano)
